...
impl ComplianceStatusRepositoryImpl {
    pub(super) async fn load_audits_impl(
        ...
        id: Uuid,
        page: PageRequest,
    ) -> Result<Page<ComplianceStatusModel>, Box<dyn Error + Send + Sync>> {
        // First, get the total count of audit records for this entity
        let count_query = r#"SELECT COUNT(*) as count FROM person_compliance_status_audit WHERE id = $1"#;
        let total: i64 = {
        ...
        let query = r#"
            SELECT * FROM person_compliance_status_audit 
            WHERE id = $1 
...
#[async_trait]
impl LoadAudits<Postgres, ComplianceStatusModel> for ComplianceStatusRepositoryImpl {
    async fn load_audits(&self, id: Uuid, page: PageRequest) -> Result<Page<ComplianceStatusModel>, Box<dyn Error + Send + Sync>> {
...
mod tests {
    ...
    async fn test_load_audits() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let mut saved = compliance_status_repo.create_batch(vec![compliance_status.clone()], Some(audit_log.id)).await?;
        ...
        for _i in 1..=3 {
            ...
            // Modify a field to create a new version - toggle sanctions_checked
            updated.sanctions_checked = !updated.sanctions_checked;
            saved = compliance_status_repo.update_batch(vec![updated], Some(audit_log.id)).await?;
        }

        // Load first page of audit records
        let page = compliance_status_repo.load_audits(compliance_status_id, PageRequest::new(2, 0)).await?;
        ...
        // Load second page
        let page2 = compliance_status_repo.load_audits(compliance_status_id, PageRequest::new(2, 2)).await?;
    ...
    async fn test_load_audits_empty() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let page = compliance_status_repo.load_audits(non_existing_id, PageRequest::new(20, 0)).await?;
