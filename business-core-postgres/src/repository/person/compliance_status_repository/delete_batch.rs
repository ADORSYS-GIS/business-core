    ...
    pub(super) async fn delete_batch_impl(
        ...
        for entity_opt in entities_to_delete {
            ...
            let entity_delete_query = sqlx::query(
                r#"
                DELETE FROM person_compliance_status WHERE id = $1
                "#,
            )
            .bind(entity.id);
...
#[async_trait]
impl DeleteBatch<Postgres> for ComplianceStatusRepositoryImpl {
...
mod tests {
    ...
    async fn test_delete_batch() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let deleted_count = compliance_status_repo.delete_batch(&ids, Some(delete_audit_log.id)).await?;

        assert_eq!(deleted_count, 1);

        // Verify deletion
        let loaded = compliance_status_repo.load_batch(&ids).await?;
