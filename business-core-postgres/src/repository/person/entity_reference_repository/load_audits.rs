...
impl EntityReferenceRepositoryImpl {
    pub(super) async fn load_audits_impl(
        ...
        id: Uuid,
        page: PageRequest,
    ) -> Result<Page<EntityReferenceModel>, Box<dyn Error + Send + Sync>> {
        // First, get the total count of audit records for this entity
        let count_query = r#"SELECT COUNT(*) as count FROM entity_reference_audit WHERE id = $1"#;
        let total: i64 = {
        ...
        let query = r#"
            SELECT * FROM entity_reference_audit 
            WHERE id = $1 
...
#[async_trait]
impl LoadAudits<Postgres, EntityReferenceModel> for EntityReferenceRepositoryImpl {
    async fn load_audits(&self, id: Uuid, page: PageRequest) -> Result<Page<EntityReferenceModel>, Box<dyn Error + Send + Sync>> {
...
mod tests {
    ...
    async fn test_load_audits() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        // Create initial entity reference
        let entity_reference = create_test_entity_reference(person_id, "EXT123");
        let entity_reference_id = entity_reference.id;
        ...
        let page = entity_reference_repo.load_audits(entity_reference_id, PageRequest::new(2, 0)).await?;
        ...
        // Load second page
        let page2 = entity_reference_repo.load_audits(entity_reference_id, PageRequest::new(2, 2)).await?;
    ...
    async fn test_load_audits_empty() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let page = entity_reference_repo.load_audits(non_existing_id, PageRequest::new(20, 0)).await?;
