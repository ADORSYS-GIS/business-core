...
mod tests {
    ...
    #[tokio::test]
    async fn test_find_ids_by_country_id() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    #[tokio::test]
    async fn test_find_ids_by_country_id_non_existing() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let ctx = setup_test_context().await?;
        let country_subdivision_repo = &ctx.person_repos().country_subdivision_repository;

        let non_existent_country_id = Uuid::new_v4();
        let found_ids = country_subdivision_repo.find_ids_by_country_id(non_existent_country_id).await?;
