...
impl PortfolioRepositoryImpl {
    pub(super) async fn load_audits_impl(
        ...
        id: Uuid,
        page: PageRequest,
    ) -> Result<Page<PortfolioModel>, Box<dyn Error + Send + Sync>> {
        // First, get the total count of audit records for this entity
        let count_query = r#"SELECT COUNT(*) as count FROM portfolio_audit WHERE id = $1"#;
        let total: i64 = {
...
#[async_trait]
impl LoadAudits<Postgres, PortfolioModel> for PortfolioRepositoryImpl {
    async fn load_audits(&self, id: Uuid, page: PageRequest) -> Result<Page<PortfolioModel>, Box<dyn Error + Send + Sync>> {
...
mod tests {
    ...
    async fn test_load_audits() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        // Create compliance status (required for portfolio)
        let compliance_status = create_test_compliance_status(person_id);
        let compliance_status_id = compliance_status.id;
        let cs_audit_log = create_test_audit_log();
        audit_log_repo.create(&cs_audit_log).await?;
        compliance_status_repo.create_batch(vec![compliance_status.clone()], Some(cs_audit_log.id)).await?;

        // Create initial portfolio
        let mut portfolio = create_test_portfolio();
        portfolio.person_id = person_id;
        portfolio.compliance_status = compliance_status_id;
        let portfolio_id = portfolio.id;
        let audit_log = create_test_audit_log();
        audit_log_repo.create(&audit_log).await?;
        let mut saved = portfolio_repo.create_batch(vec![portfolio.clone()], Some(audit_log.id)).await?;
        ...
        for i in 1..=3 {
            ...
            // Modify a field to create a new version - increment total_accounts
            updated.total_accounts = i + 1;
            saved = portfolio_repo.update_batch(vec![updated], Some(audit_log.id)).await?;
        }

        // Load first page of audit records
        let page = portfolio_repo.load_audits(portfolio_id, PageRequest::new(2, 0)).await?;
        ...
        // Load second page
        let page2 = portfolio_repo.load_audits(portfolio_id, PageRequest::new(2, 2)).await?;
    ...
    async fn test_load_audits_empty() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let page = portfolio_repo.load_audits(non_existing_id, PageRequest::new(20, 0)).await?;
