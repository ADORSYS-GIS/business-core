    ...
    pub(super) async fn create_batch_impl(
        ...
        for mut entity in items {
            ...
            let audit_link = AuditLinkModel {
                audit_log_id,
                entity_id: entity.id,
                entity_type: AuditEntityType::Portfolio,
            };
...
#[async_trait]
impl CreateBatch<Postgres, PortfolioModel> for PortfolioRepositoryImpl {
...
mod tests {
    ...
    async fn test_create_batch() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        for _i in 0..3 {
            portfolios.push(create_test_portfolio());
        }

        let saved = portfolio_repo
            .create_batch(portfolios, Some(audit_log.id))
            .await?;
