...
impl DocumentRepositoryImpl {
    pub(super) async fn load_audits_impl(
        ...
        id: Uuid,
        page: PageRequest,
    ) -> Result<Page<DocumentModel>, Box<dyn Error + Send + Sync>> {
        // First, get the total count of audit records for this entity
        let count_query = r#"SELECT COUNT(*) as count FROM person_document_audit WHERE id = $1"#;
        let total: i64 = {
        ...
        let query = r#"
            SELECT * FROM person_document_audit 
            WHERE id = $1 
...
#[async_trait]
impl LoadAudits<Postgres, DocumentModel> for DocumentRepositoryImpl {
    async fn load_audits(&self, id: Uuid, page: PageRequest) -> Result<Page<DocumentModel>, Box<dyn Error + Send + Sync>> {
...
mod tests {
    ...
    async fn test_load_audits() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let mut saved = document_repo.create_batch(vec![document.clone()], Some(audit_log.id)).await?;
        ...
        for i in 1..=3 {
            ...
            // Modify a field to create a new version - change document_path
            updated.document_path = Some(heapless::String::try_from(format!("/path/to/document_{i}").as_str()).unwrap());
            saved = document_repo.update_batch(vec![updated], Some(audit_log.id)).await?;
        }

        // Load first page of audit records
        let page = document_repo.load_audits(document_id, PageRequest::new(2, 0)).await?;
        ...
        // Load second page
        let page2 = document_repo.load_audits(document_id, PageRequest::new(2, 2)).await?;
    ...
    async fn test_load_audits_empty() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let page = document_repo.load_audits(non_existing_id, PageRequest::new(20, 0)).await?;
