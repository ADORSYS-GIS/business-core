...
impl AccountGlMappingRepositoryImpl {
    pub(super) async fn create_batch_impl(
        ...
        let audit_log_id = audit_log_id.ok_or("audit_log_id is required for AccountGlMappingModel")?;
        if items.is_empty() {
            return Ok(Vec::new());
        }
        ...
        // Acquire lock once and do all database operations
        {
            ...
            for mut item in items {
                ...
                sqlx::query(
                    ...
                    (id, customer_account_code, overdraft_code, antecedent_hash, antecedent_audit_log_id, hash, audit_log_id)
                ...
                sqlx::query(
                    r#"
                    INSERT INTO account_gl_mapping_idx (id, customer_account_code, overdraft_code)
                    VALUES ($1, $2, $3)
                    "#,
                )
                .bind(idx.id)
                .bind(idx.customer_account_code.as_str())
                .bind(idx.overdraft_code.as_deref())
                .execute(&mut **transaction)
                .await?;
...
#[async_trait]
impl CreateBatch<Postgres, AccountGlMappingModel> for AccountGlMappingRepositoryImpl {
...
mod tests {
    ...
    use business_core_db::{
        models::{index_aware::IndexAware, product::account_gl_mapping::AccountGlMappingModel},
    ...
    async fn test_create_batch() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        for i in 0..5 {
            let account_gl_mapping = create_test_account_gl_mapping(&format!("ACC{i}"));
            account_gl_mapping_entities.push(account_gl_mapping);
        }

        let saved_entities = account_gl_mapping_repo
            .create_batch(account_gl_mapping_entities.clone(), Some(audit_log.id))
            .await?;

        assert_eq!(saved_entities.len(), 5);

        for (i, saved_entity) in saved_entities.iter().enumerate() {
            assert_eq!(saved_entity.customer_account_code.as_str(), format!("ACC{i}"));
    ...
    async fn test_account_gl_mapping_insert_triggers_cache_notification(
        ...
        // Create a test account_gl_mapping entity
        let test_account_gl_mapping = create_test_account_gl_mapping(&random(20));
        let account_gl_mapping_idx = test_account_gl_mapping.to_index();
        ...
        // Insert the account_gl_mapping record
        let audit_log = create_test_audit_log();
        sqlx::query(
            r#"
        ...
        let mut test_account_gl_mapping_for_hashing = test_account_gl_mapping.clone();
        test_account_gl_mapping_for_hashing.hash = 0;
        test_account_gl_mapping_for_hashing.audit_log_id = Some(audit_log.id);
        let computed_hash =
            business_core_db::utils::hash_as_i64(&test_account_gl_mapping_for_hashing).unwrap();
        let final_account_gl_mapping = AccountGlMappingModel {
            hash: computed_hash,
            audit_log_id: Some(audit_log.id),
            ..test_account_gl_mapping
        };
        ...
        .bind(final_account_gl_mapping.id)
        .bind(final_account_gl_mapping.customer_account_code.as_str())
        .bind(final_account_gl_mapping.overdraft_code.as_deref())
        .bind(final_account_gl_mapping.antecedent_hash)
        .bind(final_account_gl_mapping.antecedent_audit_log_id)
        .bind(final_account_gl_mapping.hash)
        .bind(final_account_gl_mapping.audit_log_id)
        .execute(&**pool)
        .await
        .expect("Failed to insert account_gl_mapping");

        // Then insert the account_gl_mapping index directly into the database using raw SQL
        sqlx::query("INSERT INTO account_gl_mapping_idx (id, customer_account_code, overdraft_code) VALUES ($1, $2, $3)")
            .bind(account_gl_mapping_idx.id)
            .bind(account_gl_mapping_idx.customer_account_code.as_str())
            .bind(account_gl_mapping_idx.overdraft_code.as_deref())
            .execute(&**pool)
            .await
        ...
        assert!(
            cache.contains_primary(&account_gl_mapping_idx.id),
            "AccountGlMapping should be in cache after insert"
        );

        let cached_account_gl_mapping = cache.get_by_primary(&account_gl_mapping_idx.id);
        assert!(
            cached_account_gl_mapping.is_some(),
            "AccountGlMapping should be retrievable from cache"
        );

        // Verify the cached data matches
        let cached_account_gl_mapping = cached_account_gl_mapping.unwrap();
        assert_eq!(cached_account_gl_mapping.id, account_gl_mapping_idx.id);
        assert_eq!(cached_account_gl_mapping.customer_account_code, account_gl_mapping_idx.customer_account_code);

        // Drop the read lock before proceeding to allow notification handler to process
        drop(cache);

        // Delete the records from the database, will cascade delete account_gl_mapping_idx
