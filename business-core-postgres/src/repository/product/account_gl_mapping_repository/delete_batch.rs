    ...
    pub(super) async fn delete_batch_impl(
...

        {
            ...
            for entity in entities_to_delete.into_iter().flatten() {
                ...
                let result = sqlx::query(r#"DELETE FROM account_gl_mapping WHERE id = $1"#)
                    .bind(entity.id)
                    .execute(&mut **transaction)
                    .await?;
...
#[async_trait]
impl DeleteBatch<Postgres> for AccountGlMappingRepositoryImpl {
...
mod tests {
    ...
    async fn test_delete_batch() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        for i in 0..3 {
            let account_gl_mapping = create_test_account_gl_mapping(&format!("DEL{i}"));
        ...
        let deleted_count = account_gl_mapping_repo.delete_batch(&ids, Some(delete_audit_log.id)).await?;
    ...
    async fn test_delete_batch_with_non_existing() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let account_gl_mapping = create_test_account_gl_mapping("DEL1");
