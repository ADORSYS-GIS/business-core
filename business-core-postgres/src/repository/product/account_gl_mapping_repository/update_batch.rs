    ...
    pub(super) async fn update_batch_impl(
        ...
        
        {
            ...
            for mut item in items {
                ...
                sqlx::query(
                    r#"
                    UPDATE account_gl_mapping_idx SET
                    customer_account_code = $2,
                    overdraft_code = $3
                    WHERE id = $1
                    "#,
...
#[async_trait]
impl UpdateBatch<Postgres, AccountGlMappingModel> for AccountGlMappingRepositoryImpl {
...
mod tests {
    ...
    async fn test_update_batch() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        for i in 0..3 {
            let account_gl_mapping = create_test_account_gl_mapping(&format!("ORIG{i}"));
            account_gl_mapping_entities.push(account_gl_mapping);
        }

        let saved = account_gl_mapping_repo.create_batch(account_gl_mapping_entities, Some(audit_log.id)).await?;
        ...
        for mut account_gl_mapping in saved {
            account_gl_mapping.customer_account_code = HeaplessString::try_from("UPDATED").unwrap();
            updated_entities.push(account_gl_mapping);
        }

        let updated = account_gl_mapping_repo.update_batch(updated_entities, Some(update_audit_log.id)).await?;

        assert_eq!(updated.len(), 3);
        for account_gl_mapping in updated {
            assert_eq!(account_gl_mapping.customer_account_code.as_str(), "UPDATED");
        }
    ...
    async fn test_update_batch_empty() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let audit_log = create_test_audit_log();
        audit_log_repo.create(&audit_log).await?;
        let updated = account_gl_mapping_repo.update_batch(Vec::new(), Some(audit_log.id)).await?;
