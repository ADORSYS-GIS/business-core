...
#[async_trait]
impl ExistByIds<Postgres> for AccountGlMappingRepositoryImpl {
...
mod tests {
    ...
    async fn test_exist_by_ids() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let account_gl_mapping = create_test_account_gl_mapping("EXISTS");
        ...
        let result = account_gl_mapping_repo.exist_by_ids(&[existing_id, non_existing_id]).await?;
