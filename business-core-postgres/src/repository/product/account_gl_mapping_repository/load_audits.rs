...
impl AccountGlMappingRepositoryImpl {
    pub(super) async fn load_audits_impl(
        ...
        let count_query = r#"SELECT COUNT(*) as count FROM account_gl_mapping_audit WHERE id = $1"#;
        let total: i64 = {
        ...
        let query = r#"
            SELECT * FROM account_gl_mapping_audit 
            WHERE id = $1 
...
#[async_trait]
impl LoadAudits<Postgres, AccountGlMappingModel> for AccountGlMappingRepositoryImpl {
    async fn load_audits(&self, id: Uuid, page: PageRequest) -> Result<Page<AccountGlMappingModel>, Box<dyn Error + Send + Sync>> {
...
mod tests {
    ...
    async fn test_load_audits() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        // Create initial account_gl_mapping entity
        let account_gl_mapping = create_test_account_gl_mapping("AUDIT");
        let account_gl_mapping_id = account_gl_mapping.id;
        let audit_log = create_test_audit_log();
        audit_log_repo.create(&audit_log).await?;
        let mut saved = account_gl_mapping_repo.create_batch(vec![account_gl_mapping.clone()], Some(audit_log.id)).await?;

        // Update the account_gl_mapping entity multiple times to create audit history
        ...
        for i in 1..=3 {
            ...
            updated.customer_account_code = HeaplessString::try_from(format!("AUD{i}").as_str()).unwrap();
            saved = account_gl_mapping_repo.update_batch(vec![updated], Some(audit_log.id)).await?;
        }
        ...
        // Load second page
        let page2 = account_gl_mapping_repo.load_audits(account_gl_mapping_id, PageRequest::new(2, 2)).await?;
    ...
    async fn test_load_audits_empty() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let page = account_gl_mapping_repo.load_audits(non_existing_id, PageRequest::new(20, 0)).await?;
