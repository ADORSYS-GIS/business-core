...
impl FeeTypeGlMappingRepositoryImpl {
    pub(super) async fn load_audits_impl(
        ...
        let count_query = r#"SELECT COUNT(*) as count FROM fee_type_gl_mapping_audit WHERE id = $1"#;
        let total: i64 = {
        ...
        let query = r#"
            SELECT * FROM fee_type_gl_mapping_audit 
            WHERE id = $1 
...
#[async_trait]
impl LoadAudits<Postgres, FeeTypeGlMappingModel> for FeeTypeGlMappingRepositoryImpl {
    async fn load_audits(&self, id: Uuid, page: PageRequest) -> Result<Page<FeeTypeGlMappingModel>, Box<dyn Error + Send + Sync>> {
...
mod tests {
    ...
    async fn test_load_audits() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        // Create initial fee_type_gl_mapping entity
        let fee_type_gl_mapping = create_test_fee_type_gl_mapping("AUDIT");
        let fee_type_gl_mapping_id = fee_type_gl_mapping.id;
        let audit_log = create_test_audit_log();
        audit_log_repo.create(&audit_log).await?;
        let mut saved = fee_type_gl_mapping_repo.create_batch(vec![fee_type_gl_mapping.clone()], Some(audit_log.id)).await?;

        // Update the fee_type_gl_mapping entity multiple times to create audit history
        ...
        for i in 1..=3 {
            ...
            updated.gl_code = HeaplessString::try_from(format!("AUD{i}").as_str()).unwrap();
            saved = fee_type_gl_mapping_repo.update_batch(vec![updated], Some(audit_log.id)).await?;
        }

        // Load first page of audit records
        let page = fee_type_gl_mapping_repo.load_audits(fee_type_gl_mapping_id, PageRequest::new(2, 0)).await?;
        ...
        // Load second page
        let page2 = fee_type_gl_mapping_repo.load_audits(fee_type_gl_mapping_id, PageRequest::new(2, 2)).await?;
    ...
    async fn test_load_audits_empty() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let page = fee_type_gl_mapping_repo.load_audits(non_existing_id, PageRequest::new(20, 0)).await?;
