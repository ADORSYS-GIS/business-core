...
impl ReasonReferenceRepositoryImpl {
    pub(super) async fn delete_batch_impl(
        ...
        for entity_opt in entities_to_delete {
            ...
            .bind(final_audit_entity.reason_id)
            .bind(final_audit_entity.entity_id)
            .bind(final_audit_entity.additional_details.as_deref())
            ...
            let entity_delete_query = sqlx::query(
                r#"
                DELETE FROM reason_reference WHERE id = $1
                "#,
            )
            .bind(entity.id);
...
#[async_trait]
impl DeleteBatch<Postgres> for ReasonReferenceRepositoryImpl {
...
mod tests {
    ...
    async fn test_delete_batch() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let compliance_metadata = create_test_compliance_metadata(Some("GDPR-005"), true, false);
        compliance_metadata_repo.create_batch(vec![compliance_metadata.clone()], Some(audit_log.id)).await?;

        let reason = create_test_reason_with_compliance_metadata("DATA_ARCHIVAL", "Data Archival Reason", Some(compliance_metadata.id));
        ...
        let deleted_count = reason_reference_repo.delete_batch(&ids, Some(delete_audit_log.id)).await?;
    ...
    async fn test_delete_batch_with_non_existing() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        // Create compliance metadata and reason first
        let compliance_metadata = create_test_compliance_metadata(Some("GDPR-006"), true, false);
        compliance_metadata_repo.create_batch(vec![compliance_metadata.clone()], Some(audit_log.id)).await?;

        let reason = create_test_reason_with_compliance_metadata("DATA_DELETION", "Data Deletion Reason", Some(compliance_metadata.id));
        let saved_reasons = reason_repo.create_batch(vec![reason], Some(audit_log.id)).await?;
        let reason_id = saved_reasons[0].id;
