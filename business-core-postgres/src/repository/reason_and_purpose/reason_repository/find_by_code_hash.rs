...
mod tests {
    ...
    async fn test_find_by_code_hash() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let test_code = "FIND_BY_CODE_TEST";
        let expected_hash = hash_as_i64(&test_code).unwrap();
        ...
        let found = reason_repo.find_by_code_hash(expected_hash).await?;
        
        assert_eq!(found.len(), 3);
        for saved_reason in &saved {
            assert!(found.iter().any(|idx| idx.id == saved_reason.id));
            assert!(found.iter().all(|idx| idx.code_hash == expected_hash));
        }
    ...
    async fn test_find_by_code_hash_non_existing() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let non_existent_hash = hash_as_i64(&"NON_EXISTENT_CODE").unwrap();
        let found = reason_repo.find_by_code_hash(non_existent_hash).await?;
